...
export default function MedLinkApp() {
  ...
  const fetchReferrals = async () => {
    ...
    try {
      const resRef = await fetch(`/api/referrals?doctorId=${currentUser.id}`);
  ...
  // Fetch referrals when doctor logs in
  useEffect(() => {
    ...
    try {
      const res = await fetch("/api/appointments", {
        ...
        body: JSON.stringify({
          ...
          familyMemberId: activePatientId === currentUser?.id ? null : activePatientId,
            ...
            {!loginRole ? (
              ...
              <div className="otp-box" style={{ display: "block" }}>
                ...
                <h2 style={{ marginBottom: "0.5rem", fontSize: "1.5rem" }}>
                  {loginRole === "doctor" ? "Doctor Login" : loginRole === "patient" ? "Patient Login" : loginRole === "lab" ? "Lab Partner Login" : "Admin Panel Access"}
                </h2>
      ...
      <div id="dashboard-wrapper">
          ...
          <div>
            ...
            <ul className="sidebar-menu">
              ...
              {currentUser.role === "patient" && (
                <>
                  <li className={`sidebar-item ${activePage === "dashboard" ? "active" : ""}`}><a onClick={() => setActivePage("dashboard")}><i className="fa-solid fa-gauge"></i><span>Dashboard</span></a></li>
                  <li className={`sidebar-item ${activePage === "find-doctors" ? "active" : ""}`}><a onClick={() => setActivePage("find-doctors")}><i className="fa-solid fa-user-md"></i><span>Find & Book Doctors</span></a></li>
        ...
        <main id="main-panel">
          ...
          <header id="header">
            <div>
              <h2 style={{ fontSize: "1.25rem", fontWeight: 700 }}>
                {activePage === "dashboard" ? "Dashboard Hub" : activePage === "patients-dir" ? "Clinical Patients Folders" : activePage === "patient-folder" ? "Detailed Patient Summary" : activePage === "calendar" ? "Availability Template" : activePage === "analytics" ? "Practice Business Intel" : activePage === "find-doctors" ? "Schedule consultations" : activePage === "records-wallet" ? "Personal records Wallet" : activePage === "pending-labs" ? "Diagnostic test bookings" : "Pharmacy catalog"}
              </h2>
              <span style={{ fontSize: "0.75rem", color: "var(--text-secondary)" }}>
                {currentUser.role.toUpperCase()} PORTAL / {activePage.toUpperCase()}
              </span>
            </div>
          ...
          <div id="workspace">
                ...
                <div className="stats-grid">
                  <div className="stat-card">
                    ...
                    <div className="stat-info">
                      <h3>{appointments.filter(a => a.doctorId === currentUser.id && a.appointmentDate === new Date().toISOString().split("T")[0]).length}</h3>
                  ...
                  <div className="stat-card">
                    ...
                    <div className="stat-info">
                      <h3>{[...new Set(appointments.filter(a => a.doctorId === currentUser.id).map(a => a.patientId))].length}</h3>
                  ...
                  <div className="stat-card">
                     ...
                     <div className="stat-info">
                       <h3>₹{appointments.filter(a => a.doctorId === currentUser.id && a.status === "completed").length * (doctors.find(d => d.id === currentUser.id)?.consultationFee || 0)}</h3>
                       <p>Earnings ledger</p>
                     </div>
                   </div>
                </div>
                ...
                <div className="dash-row">
                  <div className="panel">
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "1.25rem", padding: "1.25rem 0" }}>
                      {appointments.filter(a => a.doctorId === currentUser.id).length === 0 ? (
                        <p style={{ textAlign: "center", color: "var(--text-secondary)", padding: "2rem 0", gridColumn: "span 3" }}>
                          No scheduled consults.
                        </p>
                      ) : (
                        appointments.filter(a => a.doctorId === currentUser.id).map(app => {
                          const pat = patients.find(p => p.id === app.patientId);
                          let pName = pat?.name || "Patient";
                          let pMeta = pat ? `${getPatientAge(pat.dateOfBirth)} Yrs / ${pat.gender}` : "";

                          if (app.familyMemberId) {
                          ...
                          const statusBadgeClass = app.status === "completed" 
                            ? "badge-success" 
                            ...
                            >
                              <div>
                                ...
                                <div style={{ marginBottom: "0.75rem" }}>
                                  <h4 style={{ margin: 0, color: "var(--primary)", fontSize: "1.1rem" }}>{pName}</h4>
                                ...
                                <div style={{ fontSize: "0.8rem", color: "var(--text-secondary)", backgroundColor: "var(--background)", padding: "0.5rem 0.75rem", borderRadius: "6px", border: "1px solid var(--border)", marginBottom: "1.25rem", minHeight: "45px", display: "flex", alignItems: "center" }}>
                                  <div><strong>Complaint:</strong> {app.chiefComplaint}</div>
                                </div>
                              </div>
            ...
            {currentUser.role === "doctor" && activePage === "patients-dir" && (
              <section className="page-section">
                <div className="panel">
                  <div className="panel-header">
                    <h3 className="panel-title"><i className="fa-solid fa-users" style={{ color: "var(--primary)" }}></i> Attending Clinical Patients Folder</h3>
                      ...
                      <tbody>
                        {getFilteredPatients().map(pat => (
                          <tr key={pat.id}>
                            ...
                            <td style={{ fontWeight: 600, color: "var(--primary)" }}>{pat.name}</td>
                            <td>{getPatientAge(pat.dateOfBirth)} Yrs / {pat.gender}</td>
                            <td>{pat.primaryPhone}</td>
                            <td><span className="badge badge-info">{pat.bloodGroup}</span></td>
                            <td>
                              <div style={{ fontSize: "0.75rem", fontWeight: 600 }}>Allergies: <span style={{ color: "var(--danger)" }}>{pat.allergies || "None"}</span></div>
                              <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)" }}>Chronic: {pat.chronicConditions || "None"}</div>
                            </td>
                            <td>
                              <button className="btn btn-outline btn-sm" onClick={() => { setSelectedPatDetailId(pat.id); setActivePage("patient-folder"); }}>
                                <i className="fa-solid fa-folder-open"></i> Open History
                              </button>
                            </td>
                          </tr>
                        ))}
            ...
            {currentUser.role === "doctor" && activePage === "patient-folder" && (
              <section className="page-section">
                <div style={{ marginBottom: "1.5rem" }}>
                    ...
                    <div className="dash-row">
                      <div className="panel">
                        ...
                        <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
                          <div style={{ width: 70, height: 70, borderRadius: "50%", background: "var(--primary-light)", color: "var(--primary)", fontSize: "1.75rem", display: "inline-flex", alignItems: "center", justifyContent: "center", fontWeight: 700, marginBottom: "0.5rem" }}>
                            {pat.name.charAt(0)}
                          </div>
                          <h2>{pat.name}</h2>
                          <code>{pat.id}</code>
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", fontSize: "0.85rem", borderTop: "1px solid var(--border)", paddingTop: "1rem" }}>
                          ...
                          <div><strong>Emergency:</strong> {pat.emergencyContact || "N/A"}</div>
                          <div style={{ color: "var(--danger)" }}><strong>Allergies:</strong> {pat.allergies || "No allergies logged"}</div>
                          <div><strong>Chronic Profile:</strong> {pat.chronicConditions || "No chronic diseases logged"}</div>
                        </div>
                      </div>

                      <div className="panel">
                        <div className="panel-header">
                          <h3 className="panel-title"><i className="fa-solid fa-timeline"></i> Medical History Timeline</h3>
                        ...
                        <div className="timeline">
                          {consults.map(c => (
                            <div className="timeline-item" key={c.id}>
                              <div className="timeline-date">{formatDate(c.appointmentDate)}</div>
                              <div className="timeline-title">Physician Consultation Review</div>
                              <div className="timeline-content">
                                <div><strong>Chief Complaint:</strong> {c.chiefComplaint}</div>
                                <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", marginTop: "0.25rem" }}>Plan notes: {c.notes || "None"}</div>
                              </div>
                            </div>
                          ))}
                          {reports.map(rep => (
                            <div className="timeline-item" key={rep.id}>
                              <div className="timeline-date">{formatDate(rep.uploadedAt)}</div>
                              <div className="timeline-title">Laboratory Diagnostic Findings: {rep.reportTitle}</div>
                              <div className="timeline-content">
                                <pre style={{ fontFamily: "monospace", fontSize: "0.75rem", padding: "0.5rem", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 4 }}>
                                  {rep.findings}
                                </pre>
                              </div>
                            </div>
                          ))}
                          {consults.length === 0 && reports.length === 0 && (
                            <p style={{ textAlign: "center", color: "var(--text-secondary)" }}>No history recorded in this folder.</p>
                          )}
                        </div>
                      </div>
                    </div>
            ...
            {currentUser.role === "doctor" && activePage === "calendar" && (
              <section className="page-section">
                <div className="panel">
                  <div className="panel-header"><h3 className="panel-title"><i className="fa-solid fa-calendar"></i> Calendar slots template</h3></div>
                  <div className="calendar-grid">
                    {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map(d => (
                      <div className="calendar-header-day" key={d}>{d}</div>
                    ))}
                    {Array.from({ length: 28 }).map((_, i) => (
                      <div className="calendar-day" key={i} onClick={() => alert("Slot templates loaded. Clicked!")}>
                        <span className="calendar-day-num">{i + 1}</span>
                        <div className="calendar-day-events">
                          <span className="cal-event-dot blue"></span>
                          <span className="cal-event-dot green"></span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {(() => {
                    ...
                    return pat ? (
                      <div style={{ display: "flex", gap: "2rem", fontSize: "0.85rem", color: "#1e40af" }}>
                        <div><strong>Blood Group:</strong> {pat.bloodGroup}</div>
                        <div><strong>Allergies:</strong> <span style={{ color: "var(--danger)", fontWeight: 600 }}>{pat.allergies || "None"}</span></div>
                        <div><strong>Chronic:</strong> {pat.chronicConditions || "None"}</div>
                      </div>
                    ) : null;
                  })()}
                </div>
                  ...
                  <div className="panel">
                    <div className="panel-header">
                      ...
                      <button className="btn btn-primary btn-sm" onClick={() => setActivePage("find-doctors")}>
                        <i className="fa-solid fa-plus"></i> Book Consultation
                      </button>
                    </div>
                    <div>
                      {appointments.filter(a => a.patientId === currentUser.id && a.status === "scheduled").length === 0 ? (
                        ...
                        appointments.filter(a => a.patientId === currentUser.id && a.status === "scheduled").map(app => {
                            ...
                            <div className="panel" key={app.id} style={{ borderLeft: "4px solid var(--primary)", padding: "1rem", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
                              <div>
                                ...
                                <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)" }}>
                                  Clinic: {doc?.clinicName} | Fee: ₹{doc?.consultationFee}
                                </div>
                              </div>
                    ...
                    <div className="panel">
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                        <button className="btn btn-outline" style={{ justifyContent: "flex-start", width: "100%" }} onClick={() => setActivePage("find-doctors")}>
                          <i className="fa-solid fa-user-doctor" style={{ color: "var(--primary)" }}></i> Book Doctors
                        </button>
                      ...
                      <div className="doctor-search-card" key={doc.id}>
                        <div>
                          ...
                          <div className="doc-card-details">
                            ...
                            <div><i className="fa-solid fa-indian-rupee-sign"></i> Consultation Fee: <strong>₹{doc.consultationFee}</strong></div>
                          </div>
                        </div>
                        <button className="btn btn-primary" style={{ width: "100%", justifyContent: "center" }} onClick={() => {
                          setBookingDoctorId(doc.id);
                          // Setup tomorrow date booking defaults
            ...
            {currentUser.role === "patient" && activePage === "records-wallet" && (() => {
              const combinedRecords: any[] = [
                ...labReports.filter(r => r.patientId === currentUser.id).map(r => ({
                  id: r.id,
                  type: "report",
          ...
          <form onSubmit={handleBookAppointment}>
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <textarea rows={2} placeholder="Describe reasons for scheduling consultation..." value={bookingComplaint} onChange={(e) => setBookingComplaint(e.target.value)} required></textarea>
              </div>
            </div>
            <div className="modal-footer">
  ...
  function openBookAppointmentModal() {
    setBookingDoctorId(currentUser?.id || "");
    setBookingPatientId("");
